  ...
  for t in xrange(tiles):
    if t not in skip:
      ...
      if not os.path.exists(target):
        ...
        env.Command(target,[],[cmd+' --width %d --height %d --tile-size %d --tile %d -o $TARGET'%(shape[0],shape[1],tile,t)])
...
tiled('dragon-gold',path=range(10)+[30,36,37,38,42,43,44,46,47],
...
for view in 'front back other side'.split():
  ...
  command='./render-dragon --view gosper-%s --samples 512 --data poster-gosper-%s --color-seed 184811'%(view,data)
  if view=='side':
